...
// DOM elements
let containerEl, dropdownEl, inputEl,
...
// stateful
let ADD_NEXT_TOKEN_TO_NEW_TOKEN_GROUP = true,
  AJAX_BUFFER_TIMEOUT,
  AJAX_OBJECT = {},
...
const parseSemVer = function(version) {
...
const compareSemVer = function(version, minimum) {
...
const isArray = Array.isArray || function(vArg) {
...
const tmpl = function(str, obj, htmlEscape) {
  if (htmlEscape !== true) {
    htmlEscape = false;
  }
...
const storeInCache = function(key, data, duration) {
  ...
  let localStorageSucceeded = false;
...
const validListName = function(name) {
  return (typeof name === 'string' && name !== '');
};

const validTokenGroupIndex = function(index) {
...
const validTokenGroup = function(tokenGroup) {
...
const validOptionObject = function(option) {
...
const validListObject = function(obj) {
...
const error = function(code, msg, obj) {
  if (cfg.hasOwnProperty('showErrors') !== true ||
      cfg.showErrors === false) {
    return;
  }

  let errorText = `AutoComplete Error ${  code  }: ${  msg}`;
...
const expandTokenObject = function(token) {
  if (typeof token === 'string') {
    token = {
...
const expandTokenGroup = function(group) {
...
const expandValue = function(value) {
  // single token
  if (validToken(value) === true) {
...
const expandOptionObject = function(option, parentList) {
...
const expandListObject = function(list) {
...
var expandConfig = function() {
  ...
  */
  ...
  // initialList
  const listNames = keys(cfg.lists);
...
const buildWidget = function() {
  const html = `<div class="${  CSS.autocompleteContainer  }">` +
...
var buildOptionHTML = function(option, parentList) {
  ...
  if (typeof parentList.optionHTML === 'function') {
    const html = parentList.optionHTML(option);
...
const buildOption = function(option, parentList) {
  const optionId = createId();
  VISIBLE_OPTIONS[optionId] = option;

  const childrenListName = getChildrenListName(option, parentList);

  let html = `<li class="${  CSS.listItem  } ${  CSS.option  }" ` +
...
const buildOptions = function(options, parentList, appendVisibleOptions) {
...
//       code smell
const buildTokens = function(tokens) {
  let html = '';
...
const buildStringOrFunction = function(cssClass, strOrFn, inputValue) {
  let html = `<li class="${  CSS.listItem  } ${  cssClass  }">`;
  const type = typeof strOrFn;
  if (type === 'string') {
    html += strOrFn;
  }
  if (type === 'function') {
...
const buildNoResults = function(noResultsHTML, inputValue) {
  return buildStringOrFunction(CSS.noResults, noResultsHTML, inputValue);
};

const buildLoading = function(ajaxLoadingHTML, inputValue) {
...
const moveTokenHighlightLeft = function() {
  const selectedEl = tokensEl.find(`div.${  CSS.selectedTokenGroup}`);

  // exit if there is not a selected token
  if (selectedEl.length !== 1) return;

  const prev = selectedEl.prev(`div.${  CSS.tokenGroup}`);
...
const moveTokenHighlightRight = function() {
  const selectedEl = tokensEl.find(`div.${  CSS.selectedTokenGroup}`);

  // exit if there is not a selected token
  if (selectedEl.length !== 1) return;

  const next = selectedEl.next(`div.${  CSS.tokenGroup}`);
  selectedEl.removeClass(CSS.selectedTokenGroup);
  if (next.length === 1) {
...
const positionDropdownEl = function() {
  const pos = inputEl.position();
  const height = parseInt(inputEl.height(), 10);
...
const clearTokenGroupHighlight = function() {
  tokensEl.find(`div.${  CSS.tokenGroup}`).
    removeClass(CSS.selectedTokenGroup);
};

const highlightLastTokenGroup = function() {
  tokensEl.find(`div.${  CSS.tokenGroup}`).
    removeClass(CSS.selectedTokenGroup).
    filter(':last').
    addClass(CSS.selectedTokenGroup);
...
const updateTokens = function() {
  const tokenHTML = buildTokens(TOKENS);
...
const markFirstLastOptions = function() {
  const listEls = dropdownEl.find('li');
...
const calcTextWidth = function(text) {
  ...
  const cssProps = {};
  for (let i = 0, len = CSS_TEXT_PROPS.length; i < len; i++) {
    const cssProp = CSS_TEXT_PROPS[i];
...
const adjustDropdownScroll = function() {
  // find the highlighted option
  const highlightedEl = dropdownEl.find(`li.${  CSS.highlightedOption}`);

  // exit if we did not find one
  if (highlightedEl.length !== 1) return;

  const liTop = highlightedEl.position().top;
  ...
  const liHeight = highlightedEl.height();
  const ddHeight = dropdownEl.height();
...
const setCurrentList = function(listName) {
...
const removeList = function(listName) {
...
const setValue = function(newValue) {
  ...
  if (typeof cfg.onChange === 'function') {
    const possibleNewValue = cfg.onChange(newValue, oldValue);
...
const showOptions = function(html) {
  positionDropdownEl();
...
// false otherwise
const isTokenGroupHighlighted = function() {
  return (tokensEl.find(`div.${  CSS.selectedTokenGroup}`).length === 1);
};

const removeTokenGroup = function(tokenGroupIndex) {
  ...
  const newTokens = deepCopy(TOKENS);
...
const createTokenFromOption = function(option) {
  return {
...
const addHighlightedOption = function() {
  ...
  const optionId = highlightedEl.attr('data-option-id');
  ...
  const list = cfg.lists[CURRENT_LIST_NAME];
  const option = VISIBLE_OPTIONS[optionId];
  const token = createTokenFromOption(option);
  ...
  const childrenListName = getChildrenListName(option, list);
...
const ajaxSuccess = function(data, list, inputValue, preProcess) {
...
const ajaxError = function(errType, list, inputValue) {
  // do nothing if they have cancelled autocomplete
  if (INPUT_HAS_FOCUS !== true || OPTIONS_SHOWING !== true) {
    return;
  }
  ...
  let errorMsg = `<li class="${  CSS.listItem  } ${  CSS.ajaxError  }">`;
  if (typeof list.ajaxErrorHTML === 'string') {
...
const sendAjaxRequest = function(list, inputValue) {
  ...
  const noShotGuy = ['async', 'complete', 'error', 'statusCode', 'success'];
  for (let i = 0; i < noShotGuy.length; i++) {
  ...
  AJAX_BUFFER_TIMEOUT = window.setTimeout(() => {
...
const findHTMLChars = function(str) {
...
const removeHTMLChars = function(str) {
  const chars = findHTMLChars(str);
  let str2 = '';
  for (let i = 0; i < chars.length; i++) {
...
var highlightMatchChars = function(optionHTML, input) {
  ...
  const optionHTMLArr = findHTMLChars(optionHTML);
  let optionHTML2 = '';
  for (var i = 0; i < optionHTMLArr.length; i++) {
    const c = optionHTMLArr[i].char;
    const html = optionHTMLArr[i].html;
...
const matchOptionsSpecial = function(options, input, list) {
  let i,
      len = options.length,
      options2 = [];
...
const htmlCharToSentinel = function(str) {
  str += '';
  for (let i = 0; i < HTML.length; i++) {
...
const sentinelToHtmlChar = function(str) {
  str += '';
  for (let i = 0; i < HTML.length; i++) {
...
const findCharsToMatchAgainst = function(str) {
...
const matchOptions = function(input, list) {
  const options = deepCopy(list.options);
...
const pressUpArrow = function() {
  ...
  const prevEl = highlightedEl.prevAll(`li.${  CSS.option}`).first();
  if (prevEl.length === 1) {
...
const pressDownArrow = function() {
  ...
  // get the next option
  const nextEl = highlightedEl.nextAll(`li.${  CSS.option}`).first();
  if (nextEl.length === 1) {
...
const pressEscapeKey = function() {
  hideOptions();
  clearTokenGroupHighlight();
};

const pressBackspaceOnEmptyInput = function() {
...
const pressEnterOrTab = function() {
  addHighlightedOption();
};

const pressRegularKey = function() {
  ...
  let options = matchOptions(inputValue, list);
  ...
  // build the options
  let html = buildOptions(options, list);
...
const clickContainerElement = function() {
...
const clickTokenGroup = function(e) {
...
const clickRemoveTokenGroup = function(e) {
  // prevent clickContainerEl and clickPage
  e.stopPropagation();
...
const mouseoverOption = function() {
  highlightOption(this);
};
...
const keydownWindow = function(e) {
